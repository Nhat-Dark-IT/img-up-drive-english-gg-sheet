...
class ImageProcessor:
    ...
    def enhance_with_gemini(self, text, output_path=None):
        ...
        prompt = f"""
        Hãy phân tích và chuẩn hóa danh sách từ vựng sau đây:

        Văn bản gốc:
        {text}

        Yêu cầu:
        1. Sửa lỗi chính tả trong từ vựng tiếng Anh.
        2. Bổ sung hoặc sửa từ loại chính xác (danh từ, động từ, tính từ, trạng từ, v.v.).
        3. Thêm phiên âm IPA cho từ tiếng Anh.
        4. Chuẩn hóa nghĩa tiếng Việt, bao gồm nghĩa chính xác và rõ ràng.
        5. Bổ sung nghĩa tiếng Anh, đảm bảo đầy đủ và chính xác.
        6. Thêm từ đồng nghĩa (nếu có).
        7. Tiêu đề sẽ xác định dựa vào tất cả từ vựng đã xử lý trong danh sách.

        Định dạng đầu ra cho mỗi từ:
        Tiêu đề 	Từ Vựng 	IPA	Nghĩa Tiếng Việt 	Nghĩa tiếng anh

        Ví dụ:
        Từ vựng Chuyên Ngành Công Nghệ Thông Tin  honest  /ˈɒnɪst/  trung thực  truthful, not lying  truthful, sincere

        Lưu ý:
        - Giữ nguyên số thứ tự gốc.
        - Đảm bảo đủ các trường thông tin: Tiêu đề, Từ Vựng, IPA, Nghĩa Tiếng Việt, Nghĩa Tiếng Anh, Từ đồng nghĩa.
        - Sửa các lỗi phổ biến trong từ loại, ví dụ: “quickly” không phải là một danh từ.
        - Thêm nghĩa tiếng Anh nếu thiếu, đảm bảo nghĩa không bị mơ hồ hoặc thiếu sót.
        - Bổ sung từ đồng nghĩa phù hợp nếu có.
        """
    ...
    def parse_gemini_response(self, response):
        ...
        for line in lines:
            ...
            if len(parts) >= 6:
                ...
                word = parts[1].strip()
                ipa = parts[2].strip()
                vn_meaning = parts[3].strip()
                en_meaning = parts[4].strip()
                synonyms = parts[5].strip()
                
                # Skip divider lines
                if all(c in '-—' for c in ''.join([title, word, ipa, vn_meaning, en_meaning, synonyms])):
                    continue
                ...
                # Format line without numbers
                formatted_line = f"{word}\t{ipa}\t{vn_meaning}\t{en_meaning}\t{synonyms}"
...
# tôi muốn ở đây sẽ dùng symspellpy để check lỗi ngữ pháp tiếng anh sữa lỗi chính tả , từ bị dính lại với nhau 
    # First pass: Basic cleanin
         
        # Fourth pass: Vietnamese corrections
        if self.vn_nlp:
            try:
...
def test_image_processor():
    ...
    try:
        ...
        test_image = os.path.join(test_dir, "sample1.jpg")
        ...
        # Process image
        output_path = os.path.join(output_dir, "processed_sample1.jpg")
